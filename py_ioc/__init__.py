...
import re
import yaml
from py_ioc.class_utils import parse_module_class
...
class IOC(object):
    def __init__(self):
        ...
        self.parameter_pattern = re.compile('^' + re.escape(self.parameter_prefix) + '(.+)' + re.escape(self.parameter_suffix))
    ...
    def build_service(self, name):
        ...
        if class_full_name:
            ...
            try:
                ...
                self.logger.error('Error when create service: %s. Class: %s. Arguments: %s' % (name, class_full_name, args))
                raise e
